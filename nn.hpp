...
class nn
{
 ...
 // not not a pointer                       	delete errorVector;
	...
	// operation
            void    accProgLoad(vector<float> * inputVector)
            {
                CONTEXT * pCon = stdacc;
...
///                            openHandle = clsopen(pCon, str_k_forward, CLLD_NOW);     /// string version (not done yet)
                            if(0 == networkLoaded)
                            {
                                accProgLoad(inputVector);
                                networkLoaded = 1;
				cout<<"need load network"<<endl;
                            }
			    else
			    {
				cout<<"network already loaded"<<endl;
			    }
...
//                            clGetKernelInfo(krn, CL_KERNEL_FUNCTION_NAME, sizeof(strInfo), strInfo, NULL);
 ...
 finish = clock(); 
                                ...
                                cout<<"exec time:"<<Total_time<<" seconds"<<endl;
#if 0
			    cout<<"result: "<<endl;
			    for (unsigned int outI = 0;  outI < layerNWidth(); outI++)
                            {

                                cout<<clOutputLayer[outI]<<", ";
                            }
				cout<<endl;
#endif
/// test
